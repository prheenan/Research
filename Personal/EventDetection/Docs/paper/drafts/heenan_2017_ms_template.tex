...
\firstp Atomic force microscopy (AFM) is a powerful tool for studying the mechanical properties of biological molecules.  AFM can resolve sub-nanometer molecular structure such as the double helix of DNA (XXX), lattice structure of membrane-bound proteins (YYY), and real-time motion of motor proteins (ZZZ). AFM is attractive for studies of biological systems since it operates with high resolution in a wide range of temperatures, solvents, and other environmental variables CITE() \pl

During an AFM experiment, an atomically sharp tip attached to a cantilever interacts with a sample(XXX figure). The interaction is measured by the displacement of the tip via deflection of the cantilever CITE(XXX). A calibrated tip can record interaction forces from piconewtons to nanonewtons CITE(XXX). When AFM is used as an imaging platform, the tip motion is constrained by a feedback loop \cite(XXX) and measures the features of a two-dimensional surface such as height, stiffness, and charge \cite() \cite(XXX). XXX insights \pl
