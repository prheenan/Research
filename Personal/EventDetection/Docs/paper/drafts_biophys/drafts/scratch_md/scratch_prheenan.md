...
As SMFS techniques have improved, automating the detection of rupture events has proved a critical need for reproducible analysis. For example, hundreds or thousands of unfolding rupture events are needed to accurately estimate energy landscape parameters for simple systems known to have a single barrier in their energy landscape [@dudko_theory_2008]. In addition, large numbers of events must be identified for complex systems with multiple barriers [@zhang_transformation_2013], and for experiments which refold molecules [@pierse_kinetics_2013]. Detecting these events in a reliable, reproducible way is critical for gaining insight into biomolecular systems by SMFS techniques.
