...
- A 650nm dsDNA dataset taken for this paper (see Section S{#ref_sec:SampleDetails}). 

Statistics on the data sets and the annotated events are in Section S{#ref_sec:SampleDetails}

## Description of FEATHER

 FEATHER calculates an upper bound on the probability of no event occurring at each time point, using a smoothing parameter $\tau\in[0,1]$ from the user (see Table S{#ref_tbl:Parameters}), a threshold probability defining the boundary between events and non-events, and parameters determined from the force versus time curve (see Figure S{#ref_fig:algorithm_details}). This process is illustrated in Figure {#ref_fig:flowchart} and described in more detail in Section S{#sec:DesignDetails}.
...
Table {#ref_tbl:performance} lists the performance metrics for each algorithm on the polyprotein dataset. Figure {#ref_fig:performance} shows the event detection performance of each algorithm. As defined by Table S{#ref_tbl:metrics}, relative to the best baseline FEATHER improves the relative and absolute event error by a factor of about 30 and improves the Bhattacharya coefficient's complement by a factor of about 5. Additional tests on the dsDNA dataset (See Section S{#ref_sec:datasets} and Figure S{#ref_fig:DNA}) show even greater relative performance gains. FEATHER's performance detecting events in polyprotein and dsDNA data demonstrate generalized and order-of-magnitude improvements relative to the baselines presented. 
