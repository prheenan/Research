...
As SMFS experiments and theory have improved, automating the detection of rupture events has been critical for reproducible analysis. For example, hundreds or thousands of unfolding rupture events are needed to accurately estimate energy landscape parameters for a system known to have a single energy barrier [@dudko_theory_2008]. Large numbers of events must also be obtained for complex systems with multiple barriers [@zhang_transformation_2013], and for experiments which refold molecules [@pierse_kinetics_2013]. Detecting these events in a reliable, reproducible way is critical for gaining insight into biomolecular systems by SMFS techniques.
