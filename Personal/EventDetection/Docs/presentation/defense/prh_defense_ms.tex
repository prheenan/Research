...
\invisible<-4>{\footnotetext[1]{Best et. al, \textit{J. Phys. Chem. B}, 2008}}
\end{tikzpicture}
  \end{figure}
\end{overlayarea}
}%
...
% and: tex.stackexchange.com/questions/82530/how-to-draw-a-filled-rectangle-without-a-border-using-tikz
    ...
    \node<5>[anchor=south west,inner sep=0, red,font={\large\bfseries}] at (6,2.5) {False Positives};
...
\begin{frame}{Typical examples where FEATHER has trouble}

\begin{columns}[T]
  \begin{overlayarea}{\textwidth}{\textheight}
  \begin{tikzpicture}
    \node<1>[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[width=\textwidth]{Image1533Concat2.pdf}};
    \node<2>[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[width=\textwidth]{Image3929Concat2.pdf}};
\end{tikzpicture}
\end{overlayarea}
