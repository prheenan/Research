...
\item Fit \textit{no-event} model%
  \begin{itemize}
    \item Spline fit to approach%
      \invisible<-3>{\item Residuals $r_t$}%
      \invisible<-3>{\item Yields $\epsilon,\sigma$}%
   \end{itemize}
\invisible<-4>{
...
\begin{column}{0.5\textwidth}
  \begin{overlayarea}{\textwidth}{\textheight}
  \begin{tikzpicture}
    \node[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[width=\textwidth]{algorithm_approach.pdf}};
    \begin{scope}[x={(image.south east)},y={(image.north west)}]
    ...
    \end{scope}
    \node<4>[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[width=\textwidth]{algorithm_retract.pdf}};
\end{tikzpicture}
\end{overlayarea}
\end{column}
\end{columns}
