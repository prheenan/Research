...
% column about BCC
  ...
  \item \DefineTerm{BCC}: disagreement between distributions (X,Y)
    ...
    \invisible<-5>{\item Open Fovea, Scientific Python, FEATHER: }
    \end{itemize}
  \end{itemize}
\end{column}
...
% and: tex.stackexchange.com/questions/82530/how-to-draw-a-filled-rectangle-without-a-border-using-tikz
    ...
    \begin{scope}[x={(image.south east)},y={(image.north west)}]
        ...
        \only<-5> \fill [white] (1.0 ,0.0) rectangle (1.0,1.0);%
    \end{scope}
    \only<6-7>{\node[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[width=\textwidth]{FEATHER_rupture.pdf}}};
\end{tikzpicture}
\end{overlayarea}
