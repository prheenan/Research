...
% column about BCC
  \begin{itemize}
  \item \DefineTerm{d$_{\mathrm{t}\rightarrow\mathrm{p}}$}: distance (meters) from true to predicted
  \item \DefineTerm{d$_{\mathrm{p}\rightarrow\mathrm{t}}$}: distance (meters) from predicted to true
  \end{itemize}
\end{column}
...
% column about BCC
  ...
  \item \DefineTerm{BCC}: disagreement between distributions (X,Y)
    ...
    \item 
      Open Fovea, Scientific Python, FEATHER: 
    \end{itemize}
  \end{itemize}
\end{column}
...
% see: tex.stackexchange.com/questions/9559/drawing-on-an-image-with-tikz
% and: tex.stackexchange.com/questions/82530/how-to-draw-a-filled-rectangle-without-a-border-using-tikz
    \node[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[width=\textwidth]{bcc.pdf}};
    \begin{scope}[x={(image.south east)},y={(image.north west)}]
        \only<-2> \fill [white] (0.28,0.0) rectangle (1.0,1.0);
        \only<-3> \fill [white] (0.53,0.0) rectangle (1.0,1.0);
        \only<-4> \fill [white] (0.76,0.0) rectangle (1.0,1.0);
        \only<-5> \fill [white] (1.0,1.0) rectangle (1.0,1.0);
    \end{scope}
\end{tikzpicture}
  \end{figure}
}
