...
% Your text here
  ...
  \item Residual distribution %
    ...
    \end{itemize}
  \invisible<-4>{\item FEATHER top-level%
    \begin{itemize}
      \invisible<-4>{\item Fit \textit{no-event} model%
          \begin{itemize}
          \item Spline fit to approach%
           \invisible<-5>{\item Get residuals $r_t$}%
          \invisible<-6>{\item Yields $\epsilon,\sigma$  for $r_t$}%
        \end{itemize}}
      \invisible<-7>{
...
\begin{column}{0.55\textwidth}
  ...
  \begin{tikzpicture}
    ...
    \begin{scope}[x={(image.south east)},y={(image.north west)}]
      \only<5> \fill [white] (0.0,0.0) rectangle (1.0,0.62);%
      \only<5-6> \fill [white] (0.0,0.0) rectangle (1.0,0.32);%
      \only<5-7> \fill [white] (0.0,0.0) rectangle (1.0,0.0);%
    \end{scope}
    \node<8>[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[width=\textwidth]{algorithm_retract.pdf}};
\end{tikzpicture}
\end{overlayarea}
\end{column}
\end{columns}
...
\begin{frame}{Metric}
\begin{table}
\input{../../paper/Figures/Finals/metric_table}
\end{table}
\end{frame}
